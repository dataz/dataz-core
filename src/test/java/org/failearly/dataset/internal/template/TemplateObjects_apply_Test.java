...
public class TemplateObjects_apply_Test extends TemplateObjectsTestBase {
    ...
    public void strategy_ignore__should_accept_first_but_ignore_last_template_object() throws Exception {
        ...
        assertThat(duplicated, containsInAnyOrder(
                "@org.failearly.dataset.test.MyTemplateObjectAnnotation(description=(6) On method withDuplicatedTemplateObjects, dataset=OTHER-DATASET, name=G0)",
                "@org.failearly.dataset.test.MyTemplateObjectAnnotation(description=(7) On method withDuplicatedTemplateObjects. OVERWRITE G0, dataset=SAME-DATASET, name=G0)"
        ));
        assertThat(accepted, containsInAnyOrder(
                "@org.failearly.dataset.test.MyTemplateObjectAnnotation(description=(5) On method withDuplicatedTemplateObjects, dataset=SAME-DATASET, name=G0)"
        ));
    }
    ...
    public void strategy_strict__should_throw_exception_on_first_duplicated() throws Exception {
        ...
        ExceptionVerifier.on(action).expect(DuplicateTemplateObjectException.class).expect("Duplicate template object found: @org.failearly.dataset.test.MyTemplateObjectAnnotation(description=(6) On method withDuplicatedTemplateObjects, dataset=OTHER-DATASET, name=G0)").verify();
    }
